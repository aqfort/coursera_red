...
private:
    static const int MAX_ID = 100'000;
    static const int MAX_PAGE = 1'000;
...
public:
    ...
    [[nodiscard]] double Cheer(const int& id) const {
        if (_id[id] == -1) {
            return 0.0;
        }
        ...
        return static_cast<double>(_page[0] - _page[_id[id]]) / static_cast<double>(_page[0] - 1);
    }
};
